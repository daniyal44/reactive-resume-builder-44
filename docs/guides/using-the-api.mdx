...
<Steps>
  <Step title="Sign in to the dashboard">
    Head over to [https://getownresume.netlify.app](https://getownresume.netlify.app) and sign in with your account credentials.
  </Step>
...
<Step title="Navigate to API Keys settings">
  ...
  <Step title="Open the API reference">
    ...
    <Tip>
      You can also open it directly here: [API Reference](https://docs.getownresume.netlify.app/api-reference).
    </Tip>
  ...
  <Step title="Authenticate requests with x-api-key">
    ...
    <Info>
      If you're self-hosting, replace <code>https://getownresume.netlify.app</code> with your instance URL. The API is served under <code>/api/openapi</code>.
    </Info>

    ```bash
    curl "https://getownresume.netlify.app/api/openapi/resumes" \
