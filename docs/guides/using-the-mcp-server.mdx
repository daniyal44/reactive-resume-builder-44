...
<Steps>
  ...
  <Step title="If using API key, create one">
    Head over to [https://getownresume.netlify.app/](https://getownresume.netlify.app/) (or your self-hosted instance), sign in, and navigate to **Settings → API Keys**. Click **Create a new API key**, give it a name, and copy the secret — it's only shown once.
...
```json
{
  "mcpServers": {
    "reactive-resume": {
      "url": "https://getownresume.netlify.app/mcp"
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "reactive-resume": {
      "command": "npx",
      "args": ["mcp-remote", "https://getownresume.netlify.app/mcp", "--header", "x-api-key:your-api-key"]
    }
  }
}
```
...
- The MCP endpoint is `https://getownresume.netlify.app/mcp`.
...
```bash
codex mcp add reactive-resume --url https://getownresume.netlify.app/mcp
```
...
Add `https://getownresume.netlify.app/mcp` as a custom remote MCP connector, then connect with OAuth in Claude's connector UI.
...
If you're running a self-hosted Reactive Resume instance, replace `https://getownresume.netlify.app/mcp` with your instance URL:
