...
<Info>[https://getownresume.netlify.app/schema.json](https://getownresume.netlify.app/schema.json)</Info>
...
```json
{
  "$schema": "https://getownresume.netlify.app/schema.json",
  "version": "5.0.0"
}
```
...
const ajv = new Ajv();
const schema = await fetch("https://getownresume.netlify.app/schema.json").then((r) => r.json());
...
Below is the complete JSON Schema for Reactive Resume. You can also fetch the latest version directly from [https://getownresume.netlify.app/schema.json](https://getownresume.netlify.app/schema.json).
